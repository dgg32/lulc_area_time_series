...
function generate_collection(geometry, target_index) {
  ...
  var byMonth = ee.ImageCollection(
    ee.List.sequence(0, nMonths, step).map(function (n) {
      ...
      ee.Image().addBands(-1).rename(["label_mode", "constant"]).select("label_mode").eq(0).selfMask().set('system:time_start', ini))
...
function control () {
  ...
  var targets = ['Water', 'Trees', 'Grass', 'Flooded_vegetation', 'Crops', 'Shrub_and_scrub', 'Built', 'Bare', 'Snow_and_ice'];
  var target_index = 0;
  var target = "Water"
